 ...
 */
function AccountModal({ user, lang, onClose, onUpdateName, onUpdateEmail, token }) {
  const isAr = lang === "ar";
  ...
  async function saveNewEmail() {
    ...
    if (!trimmed) { setErr(isAr ? "يرجى إدخال البريد الجديد" : "Please enter new email"); return; }
    if (trimmed === user.email) { setErr(isAr ? "البريد نفسه كالحالي" : "Same as current email"); return; }
    if (!trimmed.toLowerCase().endsWith("@gmail.com") && !trimmed.toLowerCase().endsWith("@yahoo.com")) { setErr(isAr ? "يرجى استخدام بريد @gmail.com أو @yahoo.com فقط" : "Only @gmail.com or @yahoo.com addresses are allowed"); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmed)) { setErr(isAr ? "صيغة البريد غير صحيحة" : "Invalid email format"); return; }
    setSaving(true);
    const tok = token || localStorage.getItem("bahia_token");
    if (!tok) { setErr(isAr ? "❌ انتهت الجلسة" : "❌ Session expired"); setSaving(false); return; }
    try {
      ...
      if (!res.ok) { setErr(data.detail || (isAr ? "فشل تغيير البريد" : "Failed to change email")); return; }
      setMsg(isAr ? "✅ تم تغيير البريد الإلكتروني بنجاح" : "✅ Email changed successfully");
      onUpdateEmail?.(trimmed);
      setNewEmailVal("");
    } catch { setErr(isAr ? "❌ تعذر الاتصال" : "❌ Server error"); }
  ...
  async function changePassword() {
    ...
    if (newPass.length < 8)    { setErr(isAr ? "❌ كلمة المرور يجب أن تكون 8 أحرف على الأقل." : "❌ Password must be at least 8 characters."); return; }
    if (!/[a-zA-Z\u0600-\u06FF]/.test(newPass) || !/[0-9]/.test(newPass)) { setErr(isAr ? "❌ يجب أن تحتوي كلمة المرور على حروف وأرقام معاً." : "❌ Password must contain both letters and numbers."); return; }
    if (newPass === oldPass)   { setErr(isAr ? "كلمة المرور الجديدة مطابقة للقديمة" : "New password must differ"); return; }
    setSaving(true);
    const tok = token || localStorage.getItem("bahia_token");
    if (!tok) { setErr(isAr ? "❌ انتهت الجلسة" : "❌ Session expired"); setSaving(false); return; }
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setErr(data.detail || (isAr ? "تأكد من كلمة المرور الحالية" : "Check current password")); return; }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "var(--warm)", border: "1px solid var(--border)", borderRadius: 20, width: 400, maxWidth: "94vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.22)" }}
        ...
        <div style={{ padding: "20px 28px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                ...
                {editName ? (
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    ...
                    <button onClick={saveName} style={{ padding: "9px 14px", background: "var(--pink)", border: "none", borderRadius: 8, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "var(--font)" }}>{isAr ? "حفظ" : "Save"}</button>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 12px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8 }}>
                    ...
                    <button onClick={() => setEditName(true)} style={{ background: "transparent", border: "none", color: "var(--pink)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "var(--font)" }}>{isAr ? "تعديل" : "Edit"}</button>
                  </div>
                )}
              </div>
          ...
          {tab === "email" && (
            <>
              ...
              <div>
                <div style={{ fontSize: 11, color: "var(--text-dim)", marginBottom: 4, textTransform: "uppercase", letterSpacing: ".05em" }}>
                  {isAr ? "البريد الحالي" : "Current Email"}
                </div>
                <div style={{ padding: "9px 12px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 13, color: "var(--text-muted)", direction: "ltr" }}>
                  {user.email}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  type="email"
                  value={newEmailVal}
                  onChange={e => { setNewEmailVal(e.target.value); setErr(""); setMsg(""); }}
                  placeholder=""
                  style={{ ...inputStyle }}
                />
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  background: "var(--pink)", border: "none", borderRadius: 10,
                  color: "#fff", fontWeight: 700, fontSize: 14,
                  cursor: saving || !newEmailVal.trim() ? "not-allowed" : "pointer",
                  fontFamily: "var(--font)",
                  opacity: saving || !newEmailVal.trim() ? 0.6 : 1,
                  transition: "opacity .2s",
                }}
              >
                {saving ? (isAr ? "جاري الحفظ..." : "Saving...") : (isAr ? "تغيير البريد الإلكتروني" : "Change Email")}
              </button>
            </>
          )}
          ...
          {tab === "password" && (
            <>
              {[
                [isAr ? "كلمة المرور الحالية" : "Current Password", oldPass, setOldPass],
                [isAr ? "كلمة المرور الجديدة" : "New Password",     newPass, setNewPass],
                [isAr ? "تأكيد كلمة المرور"   : "Confirm Password", confirmP, setConfirmP],
...
export default function Sidebar({
  ...
  onDeleteConv, onDeleteAllHistory, onUpdateName, onUpdateEmail, token,
}) {
  ...
  function handleDeleteAll() {
    if (confirmAll) { onDeleteAllHistory?.(); setConfirmAll(false); }
    ...
    <>
      <div className={"sb" + (open ? "" : " hidden")} dir={t.dir}>
              ...
              {items.map(item => (
                ...
                >
                  ...
                  <button
                    ...
                    style={isAr ? { left: 8, right: "auto" } : { right: 8, left: "auto" }}
                    onClick={e => { e.stopPropagation(); onDeleteConv?.(item.id); }}
                  >🗑</button>
                </div>
              ))}
            </div>
        ...
        <div className="sb-foot">
          <div className="u-row" onClick={() => setShowAccount(true)} title={isAr ? "حسابي" : "Account"}>
      ...
      {showAccount && (
        <AccountModal
          user={user} lang={lang} token={token}
