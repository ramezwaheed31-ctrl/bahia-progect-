...
export default function App() {
  ...
  const [token, setToken] = useState(
    () => localStorage.getItem("bahia_token") || null,
  );
  ...
  useEffect(() => {
    if (user) {
      ...
      setUser((u) => {
        const correctRole =
          g === "male" ? translations[lang].roleMale : translations[lang].role;
  ...
  function handleSend(text, imageFile = null) {
    ...
    const title =
      text.trim() ||
      (imageFile ? (currentLang === "ar" ? "صورة" : "Image") : "...");
    const userMsg = {
      id: Date.now(),
      role: "user",
      text,
    ...
    if (!activeChatId) {
      ...
      setHistory((prev) => {
        const next = [
          { id: activeChatId, title, date: tr.historyToday, preview: title },
          ...prev,
        ];
        if (user) LS.set("cg_hist_" + user.email, next);
        return next;
    ...
    >
      {!user ? (
        ...
        <div className={`layout ${sbOpen ? "sb-open" : "sb-closed"}`}>
          ...
          {appSplash && (
            <SplashScreen onDone={() => setAppSplash(false)} lang={lang} />
          )}

          {/* Overlay only on mobile */}
          <div
            className={"sb-overlay" + (sbOpen ? " visible" : "")}
